...
SEASON_ROUNDS = {2019: 21, 2020: 17, 2021: 22, 2022: 22, 2023: 22, 2024: 24}
...
# --- carreras (year+round -> race_name/country, la uso para el filtro por GP en Gold) ---

races_raw = _load_dim("schedule")

races = races_raw.dropDuplicates(["year", "round"]).withColumn(
    "round", F.col("round").cast(IntegerType())
)

races.write.format("delta").mode("overwrite").saveAsTable("silver_dim_races")
print(f"silver_dim_races: {races.count()} rows")
