...
def _get_credential():
    # en local usa el az login de ~/.azure (esta montado en el container)
    # en produccion con ACI entra el ManagedIdentityCredential
...
def run_notebook(workspace_id: str, notebook_id: str, parameters: dict | None = None) -> str:
    ...
    # fabric responde 202 y el Location trae la url del job instance
...
def wait_for_notebook(job_location: str, poll_interval: int = 30, timeout: int = 3600) -> str:
    ...
    while elapsed < timeout:
        ...
        # el token expira, lo refresco cada 45 min mas o menos
        if elapsed % 2700 == 0:
            token = get_fabric_token()
...
# las tablas del Warehouse (Gold) no se pueden leer por ABFSS/Delta como el Lakehouse,
# solo se exponen por el T-SQL endpoint. Hay que usar pyodbc con un token AAD sacado
# de la misma cadena de credenciales (az login en local / MSI en ACI).
SQL_COPT_SS_ACCESS_TOKEN = 1256  # atributo de pyodbc para pasar el token AAD
