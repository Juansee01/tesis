...
def make_balanced_labels(n_per_class=150) -> pd.DataFrame:
    return pd.DataFrame(
        {LABEL_COL: (["EARLY"] * n_per_class + ["MID"] * n_per_class + ["LATE"] * n_per_class)}
    )
...
def test_minimum_samples_per_class():
    ...
    for cls in VALID_CLASSES:
        assert (
            counts.get(cls, 0) >= MIN_SAMPLES_PER_CLASS
...
def test_total_samples_sufficient_for_cv():
    # con 3 clases y 5-fold CV necesito minimo 5*3=15 muestras
    df = make_balanced_labels(n_per_class=150)
    assert len(df) >= 15
