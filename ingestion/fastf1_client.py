...
class FastF1Client:
    ...
    def get_laps(self, year: int, round_number: int) -> pd.DataFrame:
        ...
        # pasar los timedelta a segundos (float) para poder guardarlos bien
    ...
    def get_weather(self, year: int, round_number: int) -> pd.DataFrame:
        ...
        # si en algun momento de la carrera llovio, la marco como mojada (any(),
        # no la primera fila: el clima puede cambiar durante la carrera)
        if "Rainfall" in weather.columns:
            weather["is_wet"] = bool(weather["Rainfall"].astype(bool).any())
    ...
    def get_schedule(self, year: int) -> pd.DataFrame:
        ...
        return schedule[[c for c in keep if c in schedule.columns]].reset_index(drop=True)
